...
describe("buildBatchQuery", function() {
  it("Should transform arrays into comma separated lists", function() {
    let batchQuery = buildBatchQuery(["fb", "SNAP"], ["chart", "news"], 5, [
      "a",
      "b"
    ]);

    expect(batchQuery).to.equal(
      "?symbols=fb,SNAP&types=chart,news&range=5&filter=a,b"
    );
  });

  it("Should drop parameters that aren't specified", function() {
    let batchQuery = buildBatchQuery(["fb", "SNAP"], ["chart", "news"]);

    expect(batchQuery).to.equal("?symbols=fb,SNAP&types=chart,news");
  });

  it("Should drop parameters specified as false", function() {
    let batchQuery = buildBatchQuery(["fb", "SNAP"], ["chart", "news"], false, [
      "b"
    ]);

    expect(batchQuery).to.equal("?symbols=fb,SNAP&types=chart,news&filter=b");
  });

  it("Should allow non-array values", function() {
    let batchQuery = buildBatchQuery("fb", "chart");
